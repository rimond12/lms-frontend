...
export default function ManageEventsPage() {
  ...
  // Fetch only events
  const { data, isLoading, refetch } = useGetItemsQuery({ category: 'event' });
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage your upcoming and past events</p>
        </div>
        <div className="flex items-center gap-3">
          <Link 
            href="/dashboard/create-new-events"
          ...
          <div className="flex items-center gap-4">
            ...
            {selectedItems.length > 0 && (
              <span className="text-sm text-gray-600">
                {selectedItems.length} events selected
              </span>
            )}
          </div>
          <button
          ...
          {items.map((item) => (
            <div key={item._id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-200">
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="rounded border-gray-300 text-black focus:ring-black"
                    />
                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium bg-gray-800 text-white">
              ...
              <div className="p-4">
                ...
                {/* Event specific info - always show for events */}
                <div className="mb-3 p-2 bg-gray-50 rounded-lg">
                  <div className="flex items-center text-xs text-gray-600 mb-2">
                    <svg className="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                    {formatDate(item.eventDate, 'MMM dd, yyyy') || 'Date TBD'}
                    {item.eventTime && ` at ${formatDate(item.eventTime, 'HH:mm') || 'Time TBD'}`}
                  </div>
                  {item.eventVenue && (
                  ...
                  {item.eventAttendees && (
                    <div className="text-xs text-gray-500">
                      👥 Expected: {item.eventAttendees} attendees
                    </div>
                  )}
                </div>

                <div className="flex items-center justify-between text-xs text-gray-500">
                  <span>Created: {formatDate(item.createdAt, 'MMM dd, yyyy') || 'Date N/A'}</span>
                  <Link
...
function EditForm({ item, onSave, onCancel }: EditFormProps) {
  // Safe date formatting function (local to EditForm)
  const formatDate = (dateString: string | undefined, formatString: string) => {
    ...
    try {
      ...
      console.warn('Invalid date format in EditForm:', dateString);
      return '';
    }
  };
  ...
  const [eventDateTime, setEventDateTime] = useState(
    item.eventTime ? formatDate(item.eventTime, "yyyy-MM-dd'T'HH:mm") :
    item.eventDate ? formatDate(item.eventDate, "yyyy-MM-dd'T'HH:mm") : ''
  );
  ...
  const [timetable, setTimetable] = useState<any[]>(
    item.timetable && Array.isArray(item.timetable) && item.timetable.length > 0 
      ? item.timetable 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedItem: BlogEventNews = {
      ...
      timetable: timetable.filter(item => item.activity.trim() !== ''),
      // Always include event fields since all items are events
      ...
      <div>
        ...
        <div className="border border-gray-300 rounded-lg overflow-hidden">
          <RichTextEditor 
            value={content} 
            onChange={setContent} 
            placeholder="Edit your event content..."
            onImageUpload={handleImageUpload}
          />
        </div>
      </div>

      {/* Event-specific fields - always show since all items are events */}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="inline-flex items-center px-3 py-2 text-sm font-medium text-blue-700 bg-blue-100 border border-blue-300 rounded-lg hover:bg-blue-200 transition-colors duration-200"
              >
            ...
            {timetable.map((item, index) => (
              <div key={index} className="border border-blue-200 rounded-lg p-4 mb-4 relative bg-gradient-to-r from-blue-50 to-indigo-50">
                {timetable.length > 1 && (
                  <button
                    type="button"
                ...
                <div className="mt-4">
                  ...
                  <input
                    ...
                    placeholder="e.g., 1 hr 30 min, 5 min, 30 min"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors duration-200"
                  />
                </div>
              </div>
